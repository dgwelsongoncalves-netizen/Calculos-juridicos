...
__version__ = "2.6.6" # Add: Sucumbência recíproca (proporcionalidade) e Honorários sobre Valor da Causa (Regra STJ s/ juros)
...
def obter_fator_tjmg(df_tjmg, data_inicio, data_fim):
    ...
    if d_fim > ultimo_mes_tabela + pd.DateOffset(months=2):
        raise ValueError(f"A Tabela TJMG está defasada! Atualize a tabela para evitar prejuízos no cálculo.")
...
def calc_tjmg_leinova(df_tjmg, df_bcb, data_cm, data_juros, data_calculo):
    ...
    return fator_cm_fase1 * fator_cm_fase2, juros_fase1 + juros_fase2
...
def calc_fazenda_publica(df_bcb, data_cm, data_juros, data_calculo):
    ...
    df_ipca_e, df_poupanca, df_selic = df_bcb['IPCA_E'], df_bcb['POUPANCA'], df_bcb['SELIC']

    fator_cm_fase1, juros_fase1 = 1.0, 0.0
    ...
    if data_calc_mes >= mes_corte_ec113:
        ...
        if pd.isna(data_juros) or data_juros <= mes_corte_ec113:
            ...
            mask_selic = (df_selic.index >= inicio_selic) & (df_selic.index <= data_calc_mes)
            fator_cm_fase2 = fator_cm_fase2 * (1 + df_selic.loc[mask_selic, 'SELIC']).prod()

    return fator_cm_fase1 * fator_cm_fase2, juros_fase1

def calc_leinova_pura(df_bcb, data_cm, data_juros, data_calculo):
    if df_bcb is None: return 1.0, 0.0
    df_ipca, df_tl = df_bcb['IPCA'], df_bcb['TAXA_LEGAL']
    data_cm_mes = pd.to_datetime(f"{data_cm.year}-{data_cm.month:02d}-01")
    data_calc_mes = pd.to_datetime(f"{data_calculo.year}-{data_calculo.month:02d}-01")
...
def executar_nash(caminho_entrada, arquivo_saida):
    ...
    if not all(aba in xls.sheet_names for aba in ['Parametros', 'Danos', 'Custas']):
        raise Exception("Arquivo inválido. Faltam abas obrigatórias.")
    ...
    prop_hon = parse_percent(get_param('Proporção Honorários (%)', 100))
    prop_custas = parse_percent(get_param('Proporção Custas (%)', 100))
    ...
    if pd.notna(data_propositura): todas_as_datas = pd.concat([todas_as_datas, pd.Series([data_propositura])])
    ...
    for idx, row in df_custas.iterrows():
        df_custas.at[idx, 'Desc_Regra'] = "IPCA + Taxa Legal (Juros Trânsito)" if teve_transito else "IPCA (Sem Juros)"
    ...
    if not tem_deducao:
        ...
        for idx, row in df_custas.iterrows():
            ...
            val_princ = (valor * f_cm) * prop_custas
            val_jur = val_princ * f_jur
            ...
            total_princ_custas += val_princ if not jg else 0.0
        ...
        # LÓGICA DE HONORÁRIOS (VALOR DA CAUSA vs CONDENAÇÃO)
        hon_calc_princ = 0.0
        hon_calc_juros = 0.0
        
        if base_hon == 'VALOR DA CAUSA' and valor_causa > 0 and pd.notna(data_propositura):
            if is_fazenda: f_cm_hon, _ = calc_fazenda_publica(df_bcb, data_propositura, pd.NaT, data_calculo)
            else: f_cm_hon, _ = calc_tjmg_leinova(tabela_tjmg, df_bcb, data_propositura, pd.NaT, data_calculo)
            hon_calc_princ = (valor_causa * f_cm_hon) * hon_perc
            hon_calc_juros = 0.0 # Sem juros conforme STJ
        elif hon_fixo > 0 and pd.notna(data_sentenca):
            if is_fazenda: f_cm_hon, f_jur_hon = calc_fazenda_publica(df_bcb, data_sentenca, data_transito_c if teve_transito else pd.NaT, data_calculo)
            else: f_cm_hon, f_jur_hon = calc_leinova_pura(df_bcb, data_sentenca, data_transito_c if teve_transito else pd.NaT, data_calculo)
            hon_calc_princ = hon_fixo * f_cm_hon
            hon_calc_juros = hon_calc_princ * f_jur_hon
        elif hon_fixo > 0:
            hon_calc_princ = hon_fixo
        else:
            hon_calc_princ = (subtotal_princ + subtotal_juros) * hon_perc
            
        hon_calc_princ *= prop_hon
        hon_calc_juros *= prop_hon
        ...
        gerar_laudo_excel(processo, teve_transito, jg, df_danos, df_custas, (subtotal_princ+subtotal_juros), (hon_exigivel_princ+hon_exigivel_juros), valor_multa, hon_523, total_geral, arquivo_saida, houve_inadimplemento, termo_juros_raw, [], base_hon, prop_hon, prop_custas)
    ...
    # MODO 2: COM DEDUÇÕES
        ...
        if base_hon == 'VALOR DA CAUSA' and valor_causa > 0 and pd.notna(data_propositura) and not jg:
            if is_fazenda: f_cm_hon, _ = calc_fazenda_publica(df_bcb, data_propositura, pd.NaT, data_corte)
            else: f_cm_hon, _ = calc_tjmg_leinova(tabela_tjmg, df_bcb, data_propositura, pd.NaT, data_corte)
            ...
            hon_suc_princ = hon_fixo * prop_hon
        elif not jg:
            hon_suc_princ = ((saldo_principal + saldo_juros) * hon_perc) * prop_hon
            
        if hon_suc_princ > 0 or hon_suc_jur > 0:
            saldo_principal += hon_suc_princ
            saldo_juros += hon_suc_jur
            historico_conta_grafica.append((data_corte, f"(+) Honorários Suc. ({prop_hon*100:.0f}%)", hon_suc_princ, hon_suc_jur, 0.0, saldo_principal+saldo_juros))
        ...
        for _, row in df_deducoes.iterrows():
            ...
            if abate_princ > saldo_principal:
                ...
                saldo_principal = 0.0
                historico_conta_grafica.append((data_ded, f"(-) Bloqueio/Depósito{str_id}", 0.0, saldo_juros, valor_ded, 0.0))
                historico_conta_grafica.append((data_ded, "(!) EXCESSO / RESTITUIR", 0.0, 0.0, excesso, 0.0))
            else:
                saldo_principal -= abate_princ
                historico_conta_grafica.append((data_ded, f"(-) Bloqueio/Depósito{str_id}", saldo_principal, saldo_juros, valor_ded, saldo_principal+saldo_juros))
        ...
        gerar_laudo_excel(processo, teve_transito, jg, df_danos, df_custas, 0, 0, 0, 0, (saldo_principal+saldo_juros), arquivo_saida, houve_inadimplemento, termo_juros_raw, historico_conta_grafica, base_hon, prop_hon, prop_custas)

# --- 5. GERAÇÃO DO LAUDO FORMATADO ---
def gerar_laudo_excel(processo, teve_transito, jg, df_danos, df_custas, subtotal, hon, multa, hon_523, total, arquivo_saida, houve_inadimplemento, termo_juros_raw, historico, base_hon, prop_hon, prop_custas):
    wb = Workbook()
    ws = wb.active
    ...
    ws['D4'] = "Base Honorários:"; ws['E4'] = base_hon
    ws['D5'] = "Proporção Hon.:"; ws['E5'] = f"{prop_hon*100:.2f}%"
    ws['D6'] = "Proporção Custas:"; ws['E6'] = f"{prop_custas*100:.2f}%"
    
    for r in range(4, 7): 
        ws.cell(row=r, column=1).font = f_negrito
        ws.cell(row=r, column=4).font = f_negrito

    linha = 8
    
    ws.merge_cells(f'A{linha}:H{linha}')
    ws[f'A{linha}'] = "1. DANOS MATERIAIS E VALORES PRINCIPAIS"
    ...
    for _, r in df_custas.iterrows():
        ...
        ws.cell(row=linha, column=8, value=val_display_c).number_format = moeda; ws.cell(row=linha, column=8).border = borda
        linha += 1
    ...
    if historico:
        ...
        ws.cell(row=linha, column=8, value=saldo_final_val).number_format = moeda
        ...
        def add_total(desc, val, negrito=False, dest=False):
            ...
            ws.cell(row=linha, column=8, value=val).number_format = moeda
        ...
        add_total(f"Honorários Acumulados ({prop_hon*100:.0f}%):", hon)
        if houve_inadimplemento:
...
class NashGUI:
    def __init__(self, root):
        ...
        regras = [
            ...
            ("CG", "Amortização Conta Gráfica Automática (Aba 'Deducoes')")
        ]
