...
def executar_nash(caminho_entrada, arquivo_saida):
    ...
    def parse_percent(val):
        try:
            # Blindagem extra contra formatos "0,10" em vez de "0.10"
    ...
    # Agora tratamos campos que podem estar vazios ("") para não crashar com ValueError
    ...
    # 5.3. CÁLCULO GERAL E HONORÁRIOS
    if not tem_deducao:
        ...
        # Conta gráfica com deduções se houver
        pass
...
def gerar_laudo_excel(processo, teve_transito, jg, df_danos, df_custas, subtotal, hon, multa, hon_523, total, arquivo_saida, houve_inadimplemento, termo_juros_raw, historico, base_hon, prop_hon, prop_custas, hon_perc, hon_fixo):
    ...
    ws.cell(row=linha, column=8).font = f_negrito; ws.cell(row=linha, column=8).fill = fundo_cinza
    ...
    add_total(f"Honorários Acumulados ({f'{hon_perc*100:.0f}%' if hon_fixo==0 else 'Fixos'}):", hon)
